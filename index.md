...
In Summer and Fall 2025, I interned as an ```diff + Applied Scientist in Amazon```. Where I applied reinforcement learning and Chain-of-Thought to train a multimodal agent (MLLM) to use a set of tools (foundational models) to find visual defects in customer uploaded images. We improved the classification performance by **6.1%**. $${\color{green}Your\ green\ text\ here}$$
